 ...
 * 1) Direct access: /wizard.php?step=name
 * 2) Friendly route via .htaccess: /wizard/?step=name  -> index.php?page=wizard&step=name
 *
 * For case (2), your index.php router must include this file when page=wizard.
 */
...
// -----------------------------
// Route-safe base URL helpers
// -----------------------------

$scriptName = $_SERVER['SCRIPT_NAME'] ?? '';
$isDirectWizardPhp = str_ends_with($scriptName, 'wizard.php');

// If served directly, post back to itself; otherwise, use the pretty route.
// ROOT is assumed to exist in your project (you already use it elsewhere).
$wizardBase = $isDirectWizardPhp
  ? basename($scriptName ?: 'wizard.php')
  : (defined('ROOT') ? rtrim((string)ROOT, '/').'/wizard/' : '/wizard/');

function h(string $s): string { return htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); }

function wizard_url(string $base, string $step): string {
  $join = (str_contains($base, '?')) ? '&' : '?';
  return $base . $join . 'step=' . rawurlencode($step);
}
...
function validate_step(string $step, array $post, array &$data): array {
  ...
  if ($step === 'email') {
    ...
    if ($email === '') $errors['email'] = '*Campo vazio, preencha para continuar.';
  ...
  <div class="row w-100">
    <div class="col-lg-8 col-11 text-center mx-auto">
      ...
      <?php if ($step === 'start'): ?>
        ...
        <form method="get" action="<?=h($wizardBase)?>" class="mt-4">
      ...
      <?php elseif ($step === 'email'): ?>
        <form method="post" action="<?=h($wizardBase)?>">
          ...
          <button type="submit" name="go" value="back" class="btn btn-link mt-3" style="color:#fff !important;">
...
<?php
include '../../../../app/view/elements/site/footer.php';
